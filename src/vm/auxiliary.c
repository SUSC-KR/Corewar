...
void get_inst_args(t_inst *inst, t_carriage *carriage, t_corewar *cw)
{
    int8_t code;
...
int get_inst_argument(t_carriage *carriage, uint8_t pos, bool mod, t_corewar *cw)
{
    int address;
    int value = 0;
    t_inst *inst;
    ...
    if (carriage->args_types[pos] & T_REG)
        value = carriage->reg[cw->map[calculate_address(carriage->pc + carriage->step)] - 1];
    if (carriage->args_types[pos] & T_DIR)
    ...
    if (carriage->args_types[pos] & T_IND)
    {
        ...
        value = get_int(cw->map, carriage->pc + (mod ? address % IDX_MOD : address), DIR_SIZE);
    }
...
void write_to_bytecode(uint8_t *map, int address, int value, int size)
{
    ...
    while (size)
    {
        map[calculate_address(address + size - 1)] = (uint8_t)((value >> i) & 0xFF);
        --size;
        i += 8;
    }
}
